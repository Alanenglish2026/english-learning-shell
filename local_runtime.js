...
export function reconcile(s){const attempts=s.attempts.filter(a=>a.mode==='S');for(const a of attempts)if(a.result){a.result.evidence_source=source(a.result);a.evidence_status=classify(a);}const pending=attempts.filter(a=>!a.result);s.speaking_pending=!!pending.length||!!(s.learner_private_state.locked&&!attempts.length);s.listener_pending=!!pending.length;s.listener_closed=!!attempts.length&&!pending.length&&!s.recording_active;if(!s.listener_closed)s.final_evidence_settled=false;if(!(s.listener_closed&&s.final_evidence_settled))s.ended=false;if(s.screen===15&&!s.ended)s.screen=pending.length?12:attempts.length?13:0;return s;}
...
export class LocalRuntime {
 ...
 async execute(path,d){
  ...
  // Temporary learner media only: cleanup is committed atomically with its evidence.
  const cleaned=[];
