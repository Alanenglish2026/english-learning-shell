...
import {BUILD as CORE_BUILD} from './build.js';
const UNIT_ID=(()=>{try{return new URL(location.href).searchParams.get('unit')||'U01';}catch{return 'U01';}})();
const courseURL=p=>{let sid;try{sid=new URL(location.href).searchParams.get('session');}catch{}return './units/'+UNIT_ID+'/'+(sid?'s/'+sid+'/':'')+p;};
function returnHome(){try{capture?.cancel?.();player?.release?.();}catch{}const u=new URL('./index.html',location.href);u.searchParams.set('return','lesson');u.searchParams.set('r',Date.now().toString());location.replace(u.href);}
...
function mediaAction(label,fn){actions.replaceChildren();const b=document.createElement('button');b.className='primary';b.textContent=label;b.onclick=()=>{if(uiBusy||recordBusy)return;uiBusy=true;notice.textContent='';let task;try{task=fn();diagnostic('CLICK',{click_type:'learning',action:label});}catch(e){task=Promise.reject(e);}Promise.resolve(task).catch(fail).finally(()=>{uiBusy=false;});};actions.append(b);return b;}
...
function startWaterFromGesture(){clearInterval(poll);clearTimeout(timer);s={...s,screen:3,played:false};layout('这是水');picture('water');const advance=()=>later(()=>go(4,'meaning_water'),3000);const task=playThen('A01',advance,true);persist({screen:3,played:false}).catch(fail);return task;}
async function deviceReady(a){layout('听听刚才的声音');if(a&&!a.audio_deleted)await ownPlayback(a);else text('p','声音测试已完成。');mediaAction('继续学习',startWaterFromGesture);}
...
async function afterRecord(a){
 ...
 if(a.mode==='DEVICE'){return deviceReady(a);}
 if(a.screen===7)return go(8,'echo_word_saved');
 layout('对方递来了饮品');picture('water');await playThen('A04',()=>go(9,'echo_request_saved'));
}
async function listen(){const t=s.trial;const id=t.target==='water'?'A01':'A02';await playThen(id,async()=>{t.plays++;t.first_play=true;t.replay_count=Math.max(0,t.plays-1);t.audio_id=id;t.heard_at=Date.now();const profile=new AudioProfile(manifest.find(x=>x.audio_id===id));t.audio_profile=profile.condition();t.audio_profile.replay_count=t.replay_count;t.audio_profile.support_level=t.help_used?'clear_support':t.replay_count?'replay':'independent';t.audio_variant=t.audio_profile.variant;t.speaker_id=t.audio_profile.speaker_id;t.playback_rate=1;t.text_visible=false;await persist({trial:t});await log('L_PLAY',{modality:'L',trial_id:t.trial_id,target:t.target,audio_id:id,first_attempt:t.plays===1,replay_count:t.replay_count});showListeningChoices();});}
...
async function renderWait(a){
 ...
 if(LOCAL){action('让协助者听',()=>{location.href='./pilot.html?mode=helper&unit='+encodeURIComponent(s.unit_id)+'&session='+s.session_id},'navigation');second('没有协助者：单人自测',()=>inlineSelfCheck(a));}
...
async function render(){
 ...
 if(n===0){layout('你的第一杯饮品');text('p','听，选，再试着说。');const b=action('开始Day1',async()=>{},'navigation');b.onclick=safe(async()=>{audio.src=courseURL('assets/TEST.wav');const priming=audio.play().then(()=>audio.pause()).catch(()=>{});s=await api('start',{});await priming;await log('CLICK',{click_type:'navigation',action:'start_day1'});await render()});return;}
 ...
 if(n===2){layout('试一下声音');text('p','说“准备好了”。');const old=s.attempts.filter(a=>a.mode==='DEVICE').at(-1);if(old)return deviceReady(old);recordControl('DEVICE','按住说话');return;}
 ...
 async function boot(){const returnLink=document.querySelector('#library-link a');if(returnLink)returnLink.onclick=e=>{e.preventDefault();returnHome();};const versionLabel=document.querySelector('#lesson-build-label');if(versionLabel)versionLabel.textContent=BUILD;const v=await globalThis.fetch('./version.json',{cache:'reload'}).then(r=>r.json());if(v.shell_version!==BUILD)throw Error('版本更新还没完成，请关闭所有英语学习页面后重新打开。');manifest=await (await fetch(courseURL('audio_manifest.json'))).json();s=await api('recover',{});diagnostic('app_started',{shell_version:BUILD});diagnostic('pwa_mode',{standalone:standalone()});diagnostic(navigator.onLine===false?'offline':'online');await render();}
 safe(async()=>{try{if(LOCAL&&!globalThis.isSecureContext)throw Error('请使用HTTPS测试入口。');await boot();}catch(e){layout('暂时不能开始');notice.textContent='请回到课程首页，联网完成准备后再试。';action('返回课程',returnHome,'recovery');}})();
 ...
 if(LOCAL&&navigator.serviceWorker)navigator.serviceWorker.register('./sw.js',{scope:'./',type:'module',updateViaCache:'none'}).catch(()=>{});
