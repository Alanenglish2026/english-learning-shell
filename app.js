...
async function render(){
 ...
 async function boot(){const v=await globalThis.fetch('./version.json',{cache:'reload'}).then(r=>r.json());if(v.shell_version!==BUILD)throw Error('版本更新还没完成，请关闭所有英语学习页面后重新打开。');manifest=await (await fetch(courseURL('audio_manifest.json'))).json();s=await api('recover',{});diagnostic('app_started',{shell_version:BUILD});diagnostic('pwa_mode',{standalone:standalone()});diagnostic(navigator.onLine===false?'offline':'online');await render();}
